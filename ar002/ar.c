...
static char *usage =
    ...
    "      written by Haruhiko Okumura <okumura@matsusaka-u.ac.jp>\n"
...
int main(int argc, char *argv[])
{
    ...
    if (temp_name != NULL && count != 0) {
        ...
        /* if (ferror(outfile) || fclose(outfile) == EOF) error("Can't write"); ... causes segfault. Thanks to Kenneth Swarthout for pointing this out to me. */
        if (ferror(outfile)) error("Can't write");
